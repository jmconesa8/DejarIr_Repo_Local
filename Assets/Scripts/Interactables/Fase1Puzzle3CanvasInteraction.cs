...
public class Fase1Puzzle3CanvasInteraction : MonoBehaviour
{
    ...
    [Space(10)]
    [Header("Cuadro 3 trigger in the environment")]
    public GameObject cuadro3Trigger;
    ...
    private void Start()
    {
        if (puzzle4 != null && puzzle4.activeInHierarchy)
        {
        ...
        // Puzzle 3 exclusive
        // Enable trigger that activates cuadro 3 so player doesnt activate it beforehand
        if (cuadro3Trigger.activeSelf == true)
        {
            cuadro3Trigger.SetActive(false);
        }
    }
    ...
    public IEnumerator ScriptedSequence()
    {
        ...
        for (int i = 0; i < puzzleObjects.Length; i++)
        {
            ...
            puzzleObjects[i].GetComponent<BoxCollider>().enabled = false;
